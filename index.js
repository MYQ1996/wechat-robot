...
bot.on('message', msg => {
    switch (msg.MsgType) {
        ...
        case bot.CONF.MSGTYPE_VOICE: // 语音消息
            noMsg(msg,"语音消息")
            break;
        case bot.CONF.MSGTYPE_EMOTICON: // 自定义表情消息
            noMsg(msg,"自定义表情消息")
            break;
        case bot.CONF.MSGTYPE_MICROVIDEO: // 小视频消息
            noMsg(msg,"小视频消息")
            break;
        case bot.CONF.MSGTYPE_VIDEO: // 视频消息
            noMsg(msg,"视频消息")
            break;
    }
});
...
function imageMsgHangler(msg) {
    bot.getMsgImg(msg.MsgId).then(res => {
        fs.writeFileSync(`./img/${msg.MsgId}.jpg`, res.data)
    }).catch(err => {
        console.log(err)
    })
}
function noMsg(msg,text) {
     sendText("暂不支持"+text+"", msg);
}